...
.controller('table_15_diasCtrl', function($scope, ConexionServ, $uibModal, toastr, $state){
	...
	$scope.traeractivos=function(activo){
		consulta = 'SELECT *, rowid FROM usuarios WHERE activo=? and eliminado==0 and plan="15 Dias" and fecha_fin  <="'+ fecha +'" and fecha_fin like "%/' + mes + '/%" '
	...
	$scope.mostrarusuarios = function(plan){
		consulta = '';
		if (plan) {
			consulta = 'SELECT *, rowid FROM usuarios WHERE activo==1 and eliminado==0 and plan="15 Dias" and fecha_fin  <="'+ fecha +'" and fecha_fin like "%/' + mes + '/%" ';
		}else{
